...
Campaign Builder 的互動模組必須依照 `runtimeMode` 分層：

```text
runtimeMode="canvas"  = 編輯排版，只渲染靜態模組畫面
runtimeMode="preview" = 預覽測試，允許模組互動 runtime
runtimeMode="export"  = 正式輸出，維持匯出 HTML/CSS/JS 行為
```

Canvas 是編輯環境，不是正式互動頁面。Canvas 不初始化 KV 輪播、商品輪播、FAQ、Tabs、Accordion、Video 等模組內互動 JS；這些互動只在 Preview / Export 執行。FAQ 是例外：Canvas / Preview 都固定靜態展開，Export 才保留原生 `<details>` 開合，且 Canvas 不顯示「預覽可互動」chip。KV 輪播、商品輪播、Tabs、Accordion、Video 等互動模組必須顯示「預覽可互動」chip。使用者點擊輪播箭頭、dot、FAQ summary 等互動控制時，只顯示「此區塊含互動效果，實際操作請至「預覽」查看。」提示，不改變模組狀態。

Preview Modal 必須以 `runtimeMode="preview"` 呼叫 `ModulePreviewRenderer`；Builder Canvas 不傳入時預設為 `runtimeMode="canvas"`。Export DOM 不因這個規則改動。
...
page.modules
  ↓
ModuleRenderer runtimeMode="export"
  ↓
...
The final target remains a shared `ModuleView` / `ModuleRenderer` path using `runtimeMode="canvas" | "preview" | "export"` and module-scoped CSS fragments. Until that migration is complete, shared static rendering bridge + verifier parity is the required safety gate.
...
- `hero` / `hero-carousel` 缺圖 placeholder 必須輸出 PC / M 兩份尺寸提示，M 版畫布、預覽、匯出都要顯示 M 版建議尺寸。
- `hero-carousel` 在 Canvas runtime 不執行輪播 JS，因此 Canvas 必須隱藏 inactive carousel controls，避免左右箭頭或 dots 因缺少 runtime 高度計算而跑版；Preview / Export 才顯示並互動。
- `product-steps` 的 `image-cards` M 版必須是單欄橫列往下排列，每列圖在上、文字在下，不可改回三欄並排。
- `logo-wall` M 版必須固定一排兩個 Logo。
...
1. Extract pure `ModuleView` components for high-risk modules:
   ...
   - `product-purchase`
   - `banner-products`
2. Add `runtimeMode="canvas" | "preview" | "export"` to those ModuleView components.
