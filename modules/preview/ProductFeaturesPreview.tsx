        ...
        <div style={{ display: 'grid', gridTemplateColumns: columns, gap: isMobile ? '14px' : isCards ? '22px' : '18px', marginTop: isMobile ? '26px' : '34px' }}>
          {data.items.map((item, index) => (
            <div key={item.id} style={{ position: 'relative', minHeight: isIconText ? 112 : isCards ? 190 : isGrid6 ? 142 : 164, borderRadius: isIconText ? 999 : isCards ? 28 : 22, padding: isIconText ? '20px 24px' : isCards ? '30px 28px 28px' : '24px 22px', background: cardBg, border: isCards ? '1px solid rgba(79,70,229,0.13)' : softBorder, boxShadow: isIconText ? '0 14px 34px rgba(47,42,37,0.07)' : isCards ? '0 24px 70px rgba(79,70,229,0.11)' : premiumShadow, display: isIconText ? 'grid' : 'block', gridTemplateColumns: isIconText ? '52px 1fr' : undefined, gap: isIconText ? 16 : undefined, alignItems: isIconText ? 'center' : undefined, overflow: 'hidden', ...hoverLift }}>
              {isCards && <span style={{ position: 'absolute', right: 22, top: 20, fontSize: 12, fontWeight: 900, letterSpacing: '0.12em', color: 'rgba(79,70,229,0.36)' }}>{String(index + 1).padStart(2, '0')}</span>}
