...
export function ProductInfoPreview({ data }: { data: ProductInfoData }) {
  ...
  return (
    <section style={{ background: data.backgroundColor || '#f8fafc', padding: isMobile ? '36px 16px' : '56px 24px', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif' }}>
      <div style={{ maxWidth: '980px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : isTable ? '1fr' : 'repeat(3, 1fr)', gap: isTable ? 0 : 16, overflow: 'hidden', borderRadius: 18, border: '1px solid rgba(15,23,42,0.08)', background: '#ffffff' }}>
          {data.items.map((item) => (
