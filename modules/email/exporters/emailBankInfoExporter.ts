...
export function generateEmailBankInfoHTML(data: EmailBankInfoData, linkUrl: string): string {
  ...
  const cols = data.items.length === 1 ? 1 : 2;
  const itemWidth = cols === 1 ? '100%' : '48%';
  ...
  const cards = data.items.map((item) => {
    ...
    return `<div style="background:#ffffff;border-radius:8px;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,0.08),0 0 0 1px rgba(0,0,0,0.05);display:inline-flex;width:${itemWidth};box-sizing:border-box;margin-bottom:10px;vertical-align:top;">
